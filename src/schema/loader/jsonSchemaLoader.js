...
const loadTypes = () => {
  ...
  const versionDirs = fs.readdirSync(typeVersionsPath);
...
const loadSchemaObject = (parsedIdentifier) => {
  ...
  if (!schemaLoader.foundSchema) throw new MissingSchemaError(`No schema found for ${parsedIdentifier.identifier}`);
